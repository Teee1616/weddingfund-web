...
export default function BrideHistoryEditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setHistoryText(coupleProfile.bride_history ?? '');
      setLoading(false);
    };
  ...
  const handleSave = async () => {
    ...
    const { error: updateError } = await supabase
      .from('couple_profiles')
      .update({ bride_history: historyText })
      .eq('id', coupleProfile.id);

    if (updateError) {
      console.error(updateError);
      setErrorMsg('新婦ヒストリーの保存に失敗しました。');
      setSaving(false);
      return;
    }

    setSuccessMsg('新婦ヒストリーを保存しました。');
    setSaving(false);
  };
      ...
      <div className="max-w-3xl mx-auto py-10 px-4">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
          <p className="text-xs text-slate-600">
            新婦の生い立ちやこれまでの歩みを自由に書いてください。HISTORYセクションに反映する想定のテキストです。
          </p>
