...
export default function GroomHistoryEditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setHistoryText(coupleProfile.groom_history ?? '');
      setLoading(false);
    };
  ...
  const handleSave = async () => {
    ...
    const { error: updateError } = await supabase
      .from('couple_profiles')
      .update({ groom_history: historyText })
      .eq('id', coupleProfile.id);

    if (updateError) {
      console.error(updateError);
      setErrorMsg('新郎ヒストリーの保存に失敗しました。');
      setSaving(false);
      return;
    }

    setSuccessMsg('新郎ヒストリーを保存しました。');
    setSaving(false);
  };
      ...
      <div className="max-w-3xl mx-auto py-10 px-4">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
          <p className="text-xs text-slate-600">
            新郎の生い立ちやこれまでの歩みを自由に書いてください。HISTORYセクションに反映する想定のテキストです。
          </p>
          ...
          <textarea
            ...
            placeholder={
              '例）\n1999年千葉県に生まれる。\n小学生のころにバスケットボールと出会い、大学まで続ける。\n現在は都内で会社員として働きながら、新しい生活を準備しています。'
            }
          />
